...
//  Raketa Test Task
//
//  Created by Yevhenii Tkachenko on 18.08.2020.
...
public struct ImageInfo {
    ...
    func calculate(rect: CGRect, origin: CGPoint? = nil, imageMode: ImageMode? = nil) -> CGRect {
        ...
        case .aspectFill:
            ...
            return CGRect(x: origin?.x ?? rect.origin.x - (w - rect.width) / 2, y : origin?.y ?? rect.origin.y - (h - rect.height) / 2, width  : w, height : h
            )
        }
    }
    
    func calculateMaximumZoomScale(_ size: CGSize) -> CGFloat {
        return max(2, max(image.size.width  / size.width, image.size.height / size.height))
    }
...
open class ImageViewerController: UIViewController {
    ...
    override open func viewWillLayoutSubviews() {
        ...
        toolBar.frame = CGRect(x: 0, y: view.bounds.height - view.safeAreaInsets.bottom - 44, width: view.bounds.width, height: 44)
    }
    ...
    fileprivate func setupToolBar() {
        toolBar.items = [UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: self, action: nil),
                         UIBarButtonItem(barButtonSystemItem: .save, target: self, action: #selector(saveImage))]
    ...
    @objc fileprivate func saveImage() {
        let alert = UIAlertController(title: "Save photo to gallery?", message: nil, preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Save", style: .default) { [weak self] (action) in
            guard let self = self else { return }
            UIImageWriteToSavedPhotosAlbum(self.imageInfo.image, self, #selector(self.image(_:didFinishSavingWithError:contextInfo:)), nil)
        })
